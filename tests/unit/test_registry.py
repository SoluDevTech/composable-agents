"""Tests for DeepAgentRegistry.

Uses real YamlAgentConfigLoader with tmp_path (internal).
Uses AsyncMock for mcp_tool_loader (external).
Patches create_agent_from_config and DeepAgentRunner (external LLM boundary).
"""
...
class TestDeepAgentRegistry:
    ...
    def config_loader(self):
        """Real YamlAgentConfigLoader."""
        return YamlAgentConfigLoader()

    @pytest.fixture
    def registry(self, agents_dir, config_loader, mock_mcp_tool_loader):
    ...
    async def test_get_runner_creates_and_returns_runner(self, mock_runner_cls, mock_create, registry):
    ...
    @patch("src.infrastructure.deepagent.registry.create_agent_from_config", new_callable=AsyncMock)
    @patch("src.infrastructure.deepagent.registry.DeepAgentRunner")
    async def test_get_runner_caches_on_second_call(self, mock_runner_cls, mock_create, registry):
    ...
    async def test_get_runner_raises_on_unknown_agent(self, registry):
        ...
        with pytest.raises(AgentNotFoundError, match="Agent not found: unknown"):
            await registry.get_runner("unknown")

    @patch("src.infrastructure.deepagent.registry.create_agent_from_config", new_callable=AsyncMock)
    @patch("src.infrastructure.deepagent.registry.DeepAgentRunner")
    async def test_get_runner_passes_tracing_provider(
        self, mock_runner_cls, mock_create, agents_dir, config_loader, mock_mcp_tool_loader
    ):
        ...
        mock_runner_cls.assert_called_once_with(mock_create.return_value, tracing_provider=tracing)

    @patch("src.infrastructure.deepagent.registry.create_agent_from_config", new_callable=AsyncMock)
    @patch("src.infrastructure.deepagent.registry.DeepAgentRunner")
    async def test_get_runner_passes_mcp_tool_loader_to_factory(
        self, mock_runner_cls, mock_create, registry, mock_mcp_tool_loader
    ):
        ...
        assert kwargs.get("mcp_tool_loader") is mock_mcp_tool_loader or (
            mock_create.call_args.args[1] is mock_mcp_tool_loader
        )
    ...
    @patch("src.infrastructure.deepagent.registry.create_agent_from_config", new_callable=AsyncMock)
    @patch("src.infrastructure.deepagent.registry.DeepAgentRunner")
    async def test_close_clears_cached_runners(self, mock_runner_cls, mock_create, registry):
    ...
    @patch("src.infrastructure.deepagent.registry.create_agent_from_config", new_callable=AsyncMock)
    @patch("src.infrastructure.deepagent.registry.DeepAgentRunner")
    async def test_close_then_get_runner_recreates(self, mock_runner_cls, mock_create, registry):
