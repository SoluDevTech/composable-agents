...
Mirrors the pickpro-back JWT adapter pattern: the adapter fetches the JWKS
document via ``httpx.AsyncClient`` and caches it in an in-memory
...
class TestJwtAdapter:
    ...
    def _stub_signing_key(self, adapter: JwtAdapter, request: pytest.FixtureRequest) -> None:
        ...
        patcher_jwk = patch("src.infrastructure.auth.jwt_adapter.PyJWK.from_dict")
        ...
        patcher_header = patch(
            "src.infrastructure.auth.jwt_adapter.jwt.get_unverified_header",
    ...
    async def test_valid_token_returns_user_with_sub(self, adapter: JwtAdapter) -> None:
        ...
        # Act
        with patch("src.infrastructure.auth.jwt_adapter.jwt.decode", return_value=claims):
            result = await adapter.decode_token("some-token")
    ...
    async def test_audience_passed_to_jwt_decode(self, adapter: JwtAdapter) -> None:
        ...
        # Act
        with patch("src.infrastructure.auth.jwt_adapter.jwt.decode", return_value=claims) as mock_decode:
    ...
    async def test_invalid_audience_returns_none(self, adapter: JwtAdapter) -> None:
        # Act
        with patch(
            "src.infrastructure.auth.jwt_adapter.jwt.decode",
    ...
    async def test_malformed_claims_returns_none(self, adapter: JwtAdapter) -> None:
        ...
        # Act
        with patch("src.infrastructure.auth.jwt_adapter.jwt.decode", return_value=bad_payload):
            result = await adapter.decode_token("some-token")
    ...
    async def test_jwks_http_error_returns_none(self, adapter: JwtAdapter) -> None:
        # Arrange
        adapter._jwks_http_client = AsyncMock()
        adapter._jwks_http_client.get = AsyncMock(side_effect=httpx.ConnectError("Connection refused"))
    ...
    @pytest.mark.usefixtures("_stub_signing_key")
    async def test_cache_reset_between_decodes_refetches(self, adapter: JwtAdapter) -> None:
    ...
    async def test_decode_token_does_not_print(self, adapter: JwtAdapter) -> None:
        ...
        # Act & Assert
        with (
            patch("src.infrastructure.auth.jwt_adapter.jwt.decode", return_value=claims),
    ...
    async def test_decode_token_logs_failure_without_pii(
        ...
        with (
            ...
            caplog.at_level(logging.DEBUG, logger="src.infrastructure.auth.jwt_adapter"),
        ):
            await adapter.decode_token("some-token")
    ...
    async def test_decode_failure_logs_decode_failed_message(
        ...
        with (
            ...
            caplog.at_level(logging.WARNING, logger="src.infrastructure.auth.jwt_adapter"),
        ):
            await adapter.decode_token("some-token")

        # Assert
        assert any(LogMessage.AUTH_JWT_DECODE_FAILED in r.getMessage() for r in caplog.records)
    ...
    async def test_jwks_fetch_failure_logs_jwks_fetch_failed_message(
        ...
        # Arrange
        adapter._jwks_http_client = AsyncMock()
        adapter._jwks_http_client.get = AsyncMock(side_effect=httpx.ConnectError("boom"))

        # Act
        with caplog.at_level(logging.ERROR, logger="src.infrastructure.auth.jwt_adapter"):
            await adapter.decode_token("some-token")

        # Assert
        assert any(LogMessage.AUTH_JWKS_FETCH_FAILED in r.getMessage() for r in caplog.records)
    ...
    async def test_pyjwk_client_error_returns_none(self, adapter: JwtAdapter) -> None:
        ...
        with (
            ...
            patch(
                "src.infrastructure.auth.jwt_adapter.PyJWK.from_dict",
