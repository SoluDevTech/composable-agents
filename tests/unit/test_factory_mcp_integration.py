...
class TestFactoryMcpIntegration:
    ...
    async def test_loads_mcp_tools_when_configs_present(self, mock_create):
        """MCP tools are loaded and passed to create_deep_agent when mcp_servers is set."""
        mock_create.return_value = MagicMock()
        fake_mcp_tool = MagicMock(name="mcp_read_file")
        fake_loader = FakeMcpToolLoader(tools=[fake_mcp_tool])
        ...
        await create_agent_from_config(config, mcp_tool_loader=fake_loader)

        mock_create.assert_called_once()
        kwargs = mock_create.call_args.kwargs
        assert kwargs["tools"] == [fake_mcp_tool]

    @pytest.mark.asyncio
    @patch("src.infrastructure.deepagent.factory.create_deep_agent")
    async def test_merges_local_and_mcp_tools(self, mock_create):
    ...
    async def test_no_mcp_without_configs(self, mock_create):
        ...
        mock_create.return_value = MagicMock()
        fake_loader = FakeMcpToolLoader(tools=[MagicMock()])
...
class TestSubagentMcpIntegration:
    ...
    async def test_subagent_without_mcp_unchanged(self, mock_create):
        ...
        mock_create.return_value = MagicMock()
        fake_loader = FakeMcpToolLoader(tools=[])
