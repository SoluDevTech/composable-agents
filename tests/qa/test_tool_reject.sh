#!/usr/bin/env bash
# Test: tool call with HITL reject
set -euo pipefail
...
echo "Rejecting tool call $TOOL_CALL_ID..."
REJECT_RESPONSE=$(curl -sf "$BASE_URL/api/v1/chat/$THREAD_ID" \
  -H "Content-Type: application/json" \
  -d "{\"tool_call_id\": \"$TOOL_CALL_ID\", \"action\": \"reject\", \"reason\": \"not authorized\"}")

echo "$REJECT_RESPONSE" | jq .

# 6. Check status is completed after reject
STATUS=$(echo "$REJECT_RESPONSE" | jq -r '.status')
[ "$STATUS" = "completed" ] && pass "status=$STATUS after reject" || fail "expected status=completed, got $STATUS"
