...
Mirrors the pickpro-back JWT adapter pattern, but uses an in-memory
``cachetools.TTLCache`` (single entry, TTL 300s) instead of Valkey. The JWKS
document is fetched via ``httpx.AsyncClient`` and cached; individual signing
keys are reconstructed from the cached JWKS without any network call.
...
class JwtAdapter(JwtServicePort):
    ...
    def __init__(self, jwks_url: str, audience: str) -> None:
        ...
        self._jwks_http_client: httpx.AsyncClient | None = (
            httpx.AsyncClient(timeout=30.0, headers={"User-Agent": "composable-agents/1.0"}) if jwks_url else None
        )
    ...
    async def decode_token(self, token: str) -> User | None:
        ...
        try:
            ...
            return None
        except (jwt.ExpiredSignatureError, jwt.InvalidAlgorithmError, jwt.PyJWTError) as e:
            ...
            logger.warning("%s: %s: %s", LogMessage.AUTH_JWT_DECODE_FAILED, type(e).__name__, e)
            return None
